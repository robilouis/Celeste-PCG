...
class Load(Protocol):
    def __call__(
        self, resources: Resources, verbose: bool, hide_banner: bool, load_timeout: int
    ) -> None: ...


class ShowDoc(Protocol):
    def __call__(
        self, obj: Model, state: State, notebook_handle: CommsHandle
    ) -> CommsHandle: ...
...
def run_notebook_hook(
    ...
    **kwargs: Any,
    ...
    if notebook_type not in _HOOKS:
        raise RuntimeError(
            f"no display hook installed for notebook type {notebook_type!r}"
        )
    if _HOOKS[notebook_type][action] is None:
        raise RuntimeError(
            f"notebook hook for {notebook_type!r} did not install {action!r} action"
        )
...
def show_doc(
    obj: Model, state: State, notebook_handle: CommsHandle | None = None
) -> CommsHandle | None:
    """ """
...
def _update_notebook_url_from_env(
    notebook_url: str | ProxyUrlFunc,
) -> str | ProxyUrlFunc:
    ...
    if os.environ.get("JUPYTER_BOKEH_EXTERNAL_URL"):
        if notebook_url != DEFAULT_JUPYTER_URL:
            log.warning(
                "Environment var 'JUPYTER_BOKEH_EXTERNAL_URL' is defined. Ignoring 'notebook_url' parameter."
            )
