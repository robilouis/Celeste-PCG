...
def create_chromium_webdriver(
    extra_options: list[str] | None = None, scale_factor: float = 1
) -> WebDriver:
...
def scale_factor_less_than_web_driver_device_pixel_ratio(
    scale_factor: float, web_driver: WebDriver
) -> bool:
...
def get_web_driver_device_pixel_ratio(web_driver: WebDriver) -> float:
    ...
    """
    device_pixel_ratio: float = web_driver.execute_script(
        calculate_web_driver_device_pixel_ratio
    )
...
class _WebdriverState:
    ...
    def get(self, scale_factor: float = 1) -> WebDriver:
        if (
            ...
            or not scale_factor_less_than_web_driver_device_pixel_ratio(
    ...
    def create(
        self, kind: DriverKind | None = None, scale_factor: float = 1
    ) -> WebDriver:
