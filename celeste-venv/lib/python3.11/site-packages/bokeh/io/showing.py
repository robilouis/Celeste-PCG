...
def show(
    ...
    * Any additional keyword arguments are passed to :class:`~bokeh.server.Server` when
    ...
    if isinstance(obj, UIElement):
        return _show_with_state(
            obj, state, browser, new, notebook_handle=notebook_handle
        )

    def is_application(obj: Any) -> TypeGuard[Application]:
        return getattr(obj, "_is_a_bokeh_application_class", False)

    if is_application(obj) or callable(
        obj
        ...
        return run_notebook_hook(
            state.notebook_type, "app", obj, state, notebook_url, **kwargs
        )
...
def _show_file_with_state(
    obj: UIElement, state: State, new: BrowserTarget, controller: BrowserLike
) -> None:
    """ """
