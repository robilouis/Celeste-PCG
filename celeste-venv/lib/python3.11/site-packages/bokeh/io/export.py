...
def export_svg(
    ...
    *,
    ...
    """
    svgs = get_svg(
        obj, width=width, height=height, driver=webdriver, timeout=timeout, state=state
    )
    ...
    *,
    ...
    with _tmp_html() as tmp:
        ...
        if driver is not None:
            web_driver = driver
            if not scale_factor_less_than_web_driver_device_pixel_ratio(
                scale_factor, web_driver
            ):
                device_pixel_ratio = get_web_driver_device_pixel_ratio(web_driver)
                raise ValueError(
                    f"Expected the web driver to have a device pixel ratio greater than {scale_factor}. "
...
def wait_until_render_complete(driver: WebDriver, timeout: int) -> None:
    ...
    def is_bokeh_render_complete(driver: WebDriver) -> bool:
        return cast(
            bool, driver.execute_script("return window._bokeh_render_complete;")
        )
...
def _write_collection(
    items: list[str], filename: PathLike | None, ext: str
) -> list[str]:
    if filename is None:
