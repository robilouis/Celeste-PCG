...
def _get_save_filename(
    state: State, filename: PathLike | None
) -> tuple[PathLike, bool]:
...
def _get_save_resources(
    state: State, resources: ResourcesLike | None, suppress_warning: bool
) -> Resources:
...
def _save_helper(
    ...
    html = file_html(
        obj, resources=resources, title=title, template=template or FILE, theme=theme
    )
