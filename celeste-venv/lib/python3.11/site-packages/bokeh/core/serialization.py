...
class Serializer:
    ...
    def __init__(
        self, *, references: set[Model] = set(), deferred: bool = True
    ) -> None:
    ...
    def _encode_int(self, obj: int) -> AnyRep:
        if -_MAX_SAFE_INT < obj <= _MAX_SAFE_INT:
            ...
            warn(
                "out of range integer may result in loss of precision", BokehUserWarning
            )
    ...
    def _encode_dict(self, obj: dict[Any, Any]) -> MapRep:
        if len(obj) == 0:
            ...
            return MapRep(
                type="map",
                entries=[
                    (self.encode(key), self.encode(val)) for key, val in obj.items()
                ],
            )
    ...
    def _encode_typed_array(self, obj: TypedArray[Any]) -> TypedArrayRep:
        ...
        def dtype() -> DataType:
            if typecode == "f":
                ...
                #    return "int64"
            self.error(
                f"can't serialize array with items of type '{typecode}@{itemsize}'"
            )
...
class Deserializer:
    ...
    def __init__(
        self, references: Sequence[Model] | None = None, *, setter: Setter | None = None
    ):
    ...
    def _decode_symbol(self, obj: SymbolRep) -> float:
        name = obj["name"]
        self.error(
            f"can't resolve named symbol '{name}'"
    ...
    def _decode_object_ref(self, obj: ObjectRefRep) -> Model:
        ...
        if attributes is not None:
            decoded_attributes = {
                key: self._decode(val) for key, val in attributes.items()
            }
