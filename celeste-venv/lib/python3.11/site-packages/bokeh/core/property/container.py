...
class Seq(ContainerProperty[T]):
    ...
    def __init__(
        self,
        item_type: TypeOrInst[Property[T]],
        *,
    ...
    def validate(self, value: Any, detail: bool = True) -> None:
        ...
        if self._is_seq(value):
            ...
            msg = (
                ""
                if not detail
                else f"expected an element of {self}, got seq with invalid items {invalid!r}"
            )
...
class Dict(ContainerProperty[Any]):
    ...
    def validate(self, value: Any, detail: bool = True) -> None:
        ...
        if not isinstance(value, dict):
            raise ValueError(
                f"{expected}, got a value of type {type(value)}" if detail else ""
            )
        ...
        err = None
        if (has_bad_keys := any(bad_keys)) & (
            has_bad_key_values := any(bad_value_keys)
        ):
            err = ValueError(
                f"{exception_header} {bad_keys_str} and {bad_value_keys_str}"
            )
        elif has_bad_keys:
...
class RestrictedDict(Dict):
    ...
    def __init__(
        self, keys_type, values_type, disallow, default={}, *, help: str | None = None
    ) -> None:
        self._disallow = set(disallow)
        super().__init__(
            keys_type=keys_type, values_type=values_type, default=default, help=help
        )
...
class NonEmpty(SingleParameterizedProperty[TSeq]):
    ...
    def __init__(
        self,
        type_param: TypeOrInst[TSeq],
        *,
        default: Init[TSeq] = Intrinsic,
        help: str | None = None,
    ) -> None:
