...
class Object(Property[T]):
    ...
    def __init__(
        ...
        if not (isinstance(instance_type, (type, str)) or callable(instance_type)):
            raise ValueError(
                f"expected a type, fn() -> type, or string, got {instance_type}"
            )
    ...
    def validate(self, value: Any, detail: bool = True) -> None:
        ...
        msg = (
            ""
            if not detail
            else f"expected an instance of type {instance_type}, got {value} of type {value_type}"
        )
...
class Instance(Object[S]):
    ...
    def _assert_type(instance_type: type[Any]) -> None:
        if not (
            isinstance(instance_type, type) and issubclass(instance_type, Serializable)
        ):
            raise ValueError(
                f"expected a subclass of Serializable (e.g. HasProps), got {instance_type}"
            )
