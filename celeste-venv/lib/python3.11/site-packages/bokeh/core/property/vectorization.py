...
class Value(Serializable):
    ...
    def to_serializable(self, serializer: Serializer) -> AnyRep:
        return serializer.encode_struct(
            type="value", value=self.value, transform=self.transform, units=self.units
        )

    @classmethod
    def from_serializable(
        cls, rep: dict[str, AnyRep], deserializer: Deserializer
    ) -> Value:
        if "value" not in rep:
            deserializer.error("expected 'value' field")
        value = deserializer.decode(rep["value"])
        transform = (
            deserializer.decode(rep["transform"]) if "transform" in rep else Unspecified
        )
...
class Field(Serializable):
    ...
    def to_serializable(self, serializer: Serializer) -> AnyRep:
        return serializer.encode_struct(
            type="field", field=self.field, transform=self.transform, units=self.units
        )
...
class Expr(Serializable):
    ...
    def to_serializable(self, serializer: Serializer) -> AnyRep:
        return serializer.encode_struct(
            type="expr", expr=self.expr, transform=self.transform, units=self.units
        )
