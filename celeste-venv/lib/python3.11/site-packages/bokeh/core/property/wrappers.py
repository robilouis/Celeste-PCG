...
def notify_owner(func):
    ...
    wrapper.__doc__ = (
        f"Container method ``{func.__name__}`` instrumented to notify property owners"
    )
    return wrapper
...
class PropertyValueContainer:
    ...
    def _register_owner(
        self, owner: HasProps, descriptor: PropertyDescriptor[Any]
    ) -> None:
        self._owners.add((owner, descriptor))

    def _unregister_owner(
        self, owner: HasProps, descriptor: PropertyDescriptor[Any]
    ) -> None:
        self._owners.discard((owner, descriptor))

    def _notify_owners(
        self, old: Any, hint: DocumentPatchedEvent | None = None
    ) -> None:
...
class PropertyValueColumnData(PropertyValueDict):
    ...
    def update(self, *args, **kwargs):
        ...
        for owner, descriptor in self._owners:
            hint = ColumnDataChangedEvent(
                owner.document, owner, "data", cols=list(cols)
            )
            descriptor._notify_mutated(owner, old, hint=hint)
    ...
    def _stream(
        ...
        self._notify_owners(
            old,
            hint=ColumnsStreamedEvent(doc, source, "data", new_data, rollover, setter),
        )
    ...
    def _patch(
        ...
        self._notify_owners(
            old, hint=ColumnsPatchedEvent(doc, source, "data", patches, setter)
        )
