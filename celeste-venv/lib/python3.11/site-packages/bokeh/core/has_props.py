...
def is_DataModel(cls: type[HasProps]) -> bool:
    ...
    return (
        issubclass(cls, HasProps)
        and getattr(cls, "__data_model__", False)
        and cls != DataModel
    )
...
class _ModelResolver:
    ...
    def clear_extensions(self) -> None:
        ...
        self._known_models = {
            key: val for key, val in self._known_models.items() if not is_extension(val)
        }
...
class MetaHasProps(type):
    ...
    def __new__(
        cls, class_name: str, bases: tuple[type, ...], class_dict: dict[str, Any]
    ):
        """ """
        ...
        for name, generator in generators.items():
            ...
            for descriptor in descriptors:
                ...
                if name in class_dict:
                    raise RuntimeError(
                        f"Two property generators both created {class_name}.{name}"
                    )
    ...
    def __init__(cls, class_name: str, bases: tuple[type, ...], _) -> None:
        ...
        own_properties = {
            k: v for k, v in cls.__dict__.items() if isinstance(v, PropertyDescriptor)
        }
        ...
        unused_overrides = (
            cls.__overridden_defaults__.keys() - cls.properties(_with_props=True).keys()
        )
        if unused_overrides:
            warn(
                f"Overrides of {unused_overrides} in class {cls.__name__} does not override anything.",
                RuntimeWarning,
            )
...
class HasProps(Serializable, metaclass=MetaHasProps):
            ...
            def qualified():
                ...
                if not issubclass(cls, Qualified):
                    ...
                    if (
                        head == "bokeh"
                        or head == "__main__"
                        or "__implementation__" in cls.__dict__
                    ):
                        return model
    ...
    def __init__(self, **properties: Any) -> None:
        ...
        for name in self.properties(
            _with_props=True
        ):  # avoid set[] for deterministic behavior
    ...
    def _raise_attribute_error_with_matches(
        self, name: str, properties: Iterable[str]
    ) -> NoReturn:
    ...
    def set_from_json(
        self, name: str, value: Any, *, setter: Setter | None = None
    ) -> None:
    ...
    @classmethod
    def lookup(
        cls, name: str, *, raises: Literal[True] = True
    ) -> PropertyDescriptor[Any]: ...

    @overload
    @classmethod
    def lookup(
        cls, name: str, *, raises: Literal[False] = False
    ) -> PropertyDescriptor[Any] | None: ...

    @classmethod
    def lookup(
        cls, name: str, *, raises: bool = True
    ) -> PropertyDescriptor[Any] | None:
        ...
        if attr is not None or (attr is None and not raises):
            return attr
        raise AttributeError(
            f"{cls.__name__}.{name} property descriptor does not exist"
        )
    ...
    @lru_cache(None)
    def properties(
        cls, *, _with_props: Literal[True] = True
    ) -> dict[str, Property[Any]]: ...

    @classmethod
    @lru_cache(None)
    def properties(
        cls, *, _with_props: bool = False
    ...
    def descriptors(cls) -> list[PropertyDescriptor[Any]]:
        ...
        return [
            cls.lookup(name) for name, _ in cls.properties(_with_props=True).items()
        ]
    ...
    def dataspecs(cls) -> dict[str, DataSpec]:
        ...
        return {
            k: v
            for k, v in cls.properties(_with_props=True).items()
            if isinstance(v, DataSpec)
        }
    ...
    def query_properties_with_values(
        ...
        *,
        ...
        if include_defaults:
            ...
            # weeded out below.
            selected_keys = set(self._property_values.keys()) | set(
    ...
    def clone(self) -> HasProps:
        ...
        """
        attrs = self.properties_with_values(
            include_defaults=False, include_undefined=True
        )
        return self.__class__(
            **{key: val for key, val in attrs.items() if val is not Undefined}
        )
...
def _HasProps_to_serializable(
    cls: type[HasProps], serializer: Serializer
) -> Ref | ModelDef:
    ...
    bases: list[type[HasProps]] = [
        base for base in cls.__bases__ if issubclass(base, Model) and base != DataModel
    ]
    if len(bases) == 0:
        extends = None
    elif len(bases) == 1:
    ...
    for prop_name in cls.__properties__:
        ...
        if default is Undefined:
            ...
            prop_def = PropertyDef(
                name=prop_name, kind=kind, default=serializer.encode(default)
            )
