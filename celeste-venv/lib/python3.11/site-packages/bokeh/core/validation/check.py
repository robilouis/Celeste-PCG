...
def silence(warning: Warning, silence: bool = True) -> set[Warning]:
    ...
    if not isinstance(warning, Warning):
        raise ValueError(
            f"Input to silence should be a warning object - not of type {type(warning)}"
        )
    if silence:
...
def process_validation_issues(issues: ValidationIssues) -> None:
    ...
    warnings = [
        ...
        if not is_silenced(Warning.get_by_code(issue.code))
    ]
    ...
    if settings.validation_level() == "errors":
        if len(errors):
            raise RuntimeError(
                f"Errors encountered during validation: {error_messages}"
            )
    elif settings.validation_level() == "all":
        if len(errors) or len(warnings):
            raise RuntimeError(
                f"Errors encountered during validation: {error_messages + warning_messages}"
            )
