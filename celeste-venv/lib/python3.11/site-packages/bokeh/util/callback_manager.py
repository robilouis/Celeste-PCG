...
class PropertyCallbackManager:
    ...
    def trigger(
        ...
        if self.document is not None:
            ...
            self.document.callbacks.notify_change(
                cast(Model, self), attr, old, new, hint, setter, invoke
            )
        else:
            invoke()
...
def _check_callback(
    callback: Callable[..., Any], fargs: Sequence[str], what: str = "Callback functions"
) -> None:
