...
def external_path(file_name: str) -> Path:
    ...
    if not file_path.exists() or not file_path.is_file():
        raise RuntimeError(
            f"Could not locate external data file {file_path}. Please execute bokeh.sampledata.download()"
        )
    with open(file_path, "rb") as file:
        meta = metadata()
        known_md5 = (
            meta.get(file_name)
            or meta.get(f"{file_name}.zip")
            or meta.get(f"{splitext(file_name)[0]}.zip")
        )
        ...
        if known_md5 != local_md5:
            raise RuntimeError(
                f"External data file {file_path} is outdated. Please execute bokeh.sampledata.download()"
            )
    return file_path
...
def _bokeh_dir(create: bool = False) -> Path:
    ...
    if not bokeh_dir.exists():
        ...
        try:
            ...
            raise RuntimeError(
                f"could not create bokeh config directory at {bokeh_dir}"
            )
    else:
...
def _download_file(
    base_url: str, filename: str, data_dir: Path, progress: bool = True
) -> None:
