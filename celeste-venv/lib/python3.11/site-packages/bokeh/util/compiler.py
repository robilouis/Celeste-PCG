...
def nodejs_compile(
    code: str, lang: str = "javascript", file: str | None = None
) -> AttrDict:
...
def set_cache_hook(
    hook: Callable[[CustomModel, Implementation], AttrDict | None]
) -> None:
...
def _model_cache_no_op(
    model: CustomModel, implementation: Implementation
) -> AttrDict | None:
...
def _get_custom_models(
    models: Sequence[type[HasProps]] | None,
) -> dict[str, CustomModel] | None:
...
def _bundle_models(custom_models: dict[str, CustomModel]) -> str:
    ...
    rendered_exports = sep.join(
        _export_template % dict(name=name, module=module) for (name, module) in exports
    )
    rendered_modules = sep.join(
        _module_template % dict(module=module, source=code)
        for (module, code) in bare_modules
    )

    content = _plugin_template % dict(
        prelude=_plugin_prelude, exports=rendered_exports, modules=rendered_modules
    )
