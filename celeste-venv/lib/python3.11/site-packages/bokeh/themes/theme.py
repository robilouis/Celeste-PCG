...
class Theme:
    ...
    def __init__(
        self, filename: PathLike | None = None, json: dict[str, Any] | None = None
    ) -> None:
        ...
        for key, value in self._json["attrs"].items():
            if not isinstance(value, dict):
                raise ValueError(
                    f"theme problem: attrs.{key} should be a dictionary of properties, not {value!r}"
                )
        ...
        if "Figure" in self._json["attrs"]:
            ...
            deprecated(
                (3, 0, 0),
                "Use of 'Figure' as a key in Theme attributes",
                "'figure' (lower-case) as a key",
            )
