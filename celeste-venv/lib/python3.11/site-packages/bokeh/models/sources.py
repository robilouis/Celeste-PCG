...
if TYPE_CHECKING:
    ...
    DataDict: TypeAlias = dict[
        str, Union[Sequence[TAny], npt.NDArray[TAny], pd.Series, pd.Index]
    ]
    ...
    * A Pandas ``GroupBy`` object
    ...
    """,
        ...
        .accepts(
            ...
            lambda x: ColumnDataSource._data_from_df(x),
        )
        .accepts(
            Object("pandas.core.groupby.GroupBy"),
            lambda x: ColumnDataSource._data_from_groupby(x),
        )
        .asserts(
    ...
    @overload
    def __init__(
        self, data: DataDict | pd.DataFrame | pd.core.groupby.GroupBy, **kwargs: TAny
    ) -> None: ...
    @overload
    ...
    def _stream(
        self,
        new_data: DataDict | pd.Series | pd.DataFrame,
        ...
        if newkeys != oldkeys:
            ...
            if missing and extra:
                ...
                raise ValueError(
                    f"Must stream updates to all existing columns (missing: {', '.join(missing)})"
                )
            else:
                raise ValueError(
                    f"Must stream updates to all existing columns (extra: {', '.join(extra)})"
                )
        ...
        if needs_length_check:
            ...
            for _, x in new_data.items():
                if isinstance(x, arr_types):
                    if len(x.shape) != 1:
                        raise ValueError(
                            f"stream(...) only supports 1d sequences, got ndarray with size {x.shape!r}"
                        )
