...
def _deprecated_datetime_list_format(fmt: list[str]) -> str:
    ...
    if len(fmt) > 1:
        warn(
            f"DatetimeFormatter scales now only accept a single format. Using the first provided: {fmt[0]!r}"
        )
    return fmt[0]
...
class DatetimeTickFormatter(TickFormatter):
    ...
    microseconds = String(
        help=_DATETIME_TICK_FORMATTER_HELP("``microseconds``"), default="%fus"
    ).accepts(List(String), _deprecated_datetime_list_format)

    milliseconds = String(
        help=_DATETIME_TICK_FORMATTER_HELP("``milliseconds``"), default="%3Nms"
    ).accepts(List(String), _deprecated_datetime_list_format)

    seconds = String(
        help=_DATETIME_TICK_FORMATTER_HELP("``seconds``"), default="%Ss"
    ).accepts(List(String), _deprecated_datetime_list_format)
    ...
    minutes = String(
        help=_DATETIME_TICK_FORMATTER_HELP("``minutes``"), default=":%M"
    ).accepts(List(String), _deprecated_datetime_list_format)
    ...
    hours = String(
        help=_DATETIME_TICK_FORMATTER_HELP("``hours``"), default="%Hh"
    ).accepts(List(String), _deprecated_datetime_list_format)

    days = String(
        help=_DATETIME_TICK_FORMATTER_HELP("``days``"), default="%m/%d"
    ).accepts(List(String), _deprecated_datetime_list_format)

    months = String(
        help=_DATETIME_TICK_FORMATTER_HELP("``months``"), default="%m/%Y"
    ).accepts(List(String), _deprecated_datetime_list_format)

    years = String(
        help=_DATETIME_TICK_FORMATTER_HELP("``years``"), default="%Y"
    ).accepts(List(String), _deprecated_datetime_list_format)

    strip_leading_zeros = Bool(
        default=True, help="Whether to strip any leading zeros in the formatted ticks"
    )
...
_dttf_defaults_string = "\n\n        ".join(
    f"{name} = {_dttf_defaults[name]!r}" for name in _dttf_fields
)

DatetimeTickFormatter.__doc__ = format_docstring(
    DatetimeTickFormatter.__doc__, defaults=_dttf_defaults_string
)
