...
class Plot(LayoutDOM):
    ...
    def remove_tools(self, *tools: Tool) -> None:
        ...
        for tool in tools:
            if not isinstance(tool, Tool):
                raise ValueError(
                    "All arguments to remove_tool must be Tool subclasses."
                )
            elif tool not in self.toolbar.tools:
                raise ValueError(
                    f"Invalid tool {tool} specified. Available tools are {nice_join(self.toolbar.tools)}"
                )
    ...
    @overload
    def add_glyph(
        self, source: ColumnarDataSource, glyph: Glyph, **kwargs: Any
    ) -> GlyphRenderer: ...

    def add_glyph(
        self,
        source_or_glyph: Glyph | ColumnarDataSource,
        ...
        if self.x_scale is not None:
            for rng in x_ranges:
                if isinstance(rng, (DataRange1d, Range1d)) and not isinstance(
                    ...
                    incompatible.append(
                        f"incompatibility on x-dimension: {rng}, {self.x_scale}"
                    )
        ...
        if self.y_scale is not None:
            for rng in y_ranges:
                if isinstance(rng, (DataRange1d, Range1d)) and not isinstance(
                    ...
                    incompatible.append(
                        f"incompatibility on y-dimension: {rng}, {self.y_scale}"
                    )
                elif isinstance(rng, FactorRange) and not isinstance(
