...
class GlyphRenderer(DataRenderer):
    ...
    def _check_cdsview_filters_with_connected(self):
        if isinstance(self.glyph, ConnectedXYGlyph) and not isinstance(
            self.view.filter, AllIndices
        ):
            return str(self)
    ...
    def _check_bad_column_name(self):
        ...
        if not isinstance(source, ColumnDataSource) or isinstance(
        ...
        for spec in sorted(specs):
            if (
                isinstance(props[spec], Field)
                and (field := props[spec].field) not in colnames
            ):
    ...
    def add_decoration(
        self, marking: Marking, node: Literal["start", "middle", "end"]
    ) -> Decoration:
        glyphs = [
            self.glyph,
    ...
    def construct_color_bar(self, **kwargs: Any) -> ColorBar:
        ...
        * ``color_mapper`` for Image
        ...
        if isinstance(self.glyph, FillGlyph):
            ...
            if not (
                isinstance(fill_color, Field)
                and isinstance(fill_color.transform, ColorMapper)
            ):
                raise ValueError(
                    "expected fill_color to be a field with a ColorMapper transform"
                )
            ...
            if not (
                isinstance(line_color, Field)
                and isinstance(line_color.transform, ColorMapper)
            ):
                raise ValueError(
                    "expected line_color to be a field with a ColorMapper transform"
                )
            ...
            if not (
                isinstance(text_color, Field)
                and isinstance(text_color.transform, ColorMapper)
            ):
                raise ValueError(
                    "expected text_color to be a field with a ColorMapper transform"
                )
            ...
            raise ValueError(
                f"construct_color_bar does not handle glyph type {type(self.glyph).__name__}"
            )
