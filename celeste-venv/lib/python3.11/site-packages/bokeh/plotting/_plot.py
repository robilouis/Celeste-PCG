...
def get_range(
    range_input: (
        Range | tuple[float, float] | Sequence[str] | pd.Series[Any] | GroupBy | None
    )
) -> Range:
    import pandas as pd
    from pandas.core.groupby import GroupBy
...
def _get_axis_class(
    axis_type: AxisType | None, range_input: Range, dim: Dim
) -> tuple[type[Axis] | None, Any]:
...
def _get_num_minor_ticks(
    axis_class: type[Axis], num_minor_ticks: int | Literal["auto"] | None
) -> int:
