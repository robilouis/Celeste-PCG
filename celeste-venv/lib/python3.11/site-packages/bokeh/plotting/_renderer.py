...
def create_renderer(glyphclass, plot, **kwargs):
    ...
    incompatible_literal_spec_values += _process_sequence_literals(
        glyphclass, glyph_visuals, source, is_user_source
    )
    if incompatible_literal_spec_values:
        raise RuntimeError(
            _GLYPH_SOURCE_MSG
            % nice_join(incompatible_literal_spec_values, conjunction="and")
        )
    ...
    if any(x.startswith("selection_") for x in kwargs):
        selection_visuals = pop_visuals(
            glyphclass, kwargs, prefix="selection_", defaults=glyph_visuals
        )
    else:
    ...
    if any(x.startswith("hover_") for x in kwargs):
        hover_visuals = pop_visuals(
            glyphclass, kwargs, prefix="hover_", defaults=glyph_visuals
        )
    else:
        hover_visuals = None
...
def _process_sequence_literals(glyphclass, kwargs, source, is_user_source):
    ...
    for var, val in kwargs.items():
        ...
        if isinstance(dataspecs[var], ColorSpec) and dataspecs[
            var
        ].is_color_tuple_shape(val):
            continue
        ...
        if isinstance(val, np.ndarray):
            if isinstance(dataspecs[var], ColorSpec):
                if val.dtype == "uint32" and val.ndim == 1:  # 0xRRGGBBAA
                    ...
                    (val.dtype == "uint8" or val.dtype.kind == "f")
                    and val.ndim == 2
