...
if TYPE_CHECKING:
    ...
    ContourColorOrPalette: TypeAlias = Union[
        ContourColor, Palette, PaletteCollection, ContourColor
    ]
...
def from_contour(
    ...
    **visuals,  # This is union of LineProps, FillProps and HatchProps
    ...
    if unknown:
        raise ValueError(
            f"Unknown keyword arguments in 'from_contour': {', '.join(unknown)}"
        )

    new_contour_data = contour_data(
        x=x, y=y, z=z, levels=levels, want_fill=want_fill, want_line=want_line
    )

    # Will be other possibilities here like logarithmic....
    contour_renderer = ContourRenderer(
        fill_renderer=GlyphRenderer(
            glyph=MultiPolygons(), data_source=ColumnDataSource()
        ),
...
def _color(color: ContourColorOrPalette, n: int) -> ContourColor:
    ...
    if (
        ...
        and not isinstance(color, (bytes, str))
        and len(color) != n
    ):
...
def _contour_coords(
    ...
    cont_gen = contour_generator(
        x, y, z, line_type=LineType.ChunkCombinedOffset, fill_type=FillType.OuterOffset
    )
...
def _palette_from_collection(collection: PaletteCollection, n: int) -> Palette:
    ...
    raise ValueError(
        f"Unable to extract or interpolate palette of length {n} from PaletteCollection"
    )
