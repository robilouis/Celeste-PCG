...
def get_graph_kwargs(node_source, edge_source, **kwargs):
    ...
    if any(x.startswith("node_selection_") for x in kwargs):
        snode_visuals = pop_visuals(
            marker_type, kwargs, prefix="node_selection_", defaults=node_visuals
        )
    else:
        snode_visuals = None

    if any(x.startswith("node_hover_") for x in kwargs):
        hnode_visuals = pop_visuals(
            marker_type, kwargs, prefix="node_hover_", defaults=node_visuals
        )
    else:
        hnode_visuals = None
    ...
    mnode_visuals = pop_visuals(
        ...
        kwargs,
        prefix="node_muted_",
        defaults=node_visuals,
        override_defaults={"alpha": 0.2},
    )

    nsnode_visuals = pop_visuals(
        marker_type, kwargs, prefix="node_nonselection_", defaults=node_visuals
    )
    ...
    if any(x.startswith("edge_selection_") for x in kwargs):
        sedge_visuals = pop_visuals(
            MultiLine, kwargs, prefix="edge_selection_", defaults=edge_visuals
        )
    else:
        sedge_visuals = None

    if any(x.startswith("edge_hover_") for x in kwargs):
        hedge_visuals = pop_visuals(
            MultiLine, kwargs, prefix="edge_hover_", defaults=edge_visuals
        )
    else:
        hedge_visuals = None
    ...
    medge_visuals = pop_visuals(
        ...
        kwargs,
        prefix="edge_muted_",
        defaults=edge_visuals,
        override_defaults={"alpha": 0.2},
    )

    nsedge_visuals = pop_visuals(
        MultiLine, kwargs, prefix="edge_nonselection_", defaults=edge_visuals
    )
    ...
    node_kwargs = {
        ...
        if k.lstrip("node_") in marker_type.properties()
    }
    ...
    renderer_kwargs = {
        attr: kwargs.pop(attr) for attr in RENDERER_ARGS if attr in kwargs
    }
