...
class BokehPlotDirective(BokehDirective):
    ...
    def run(self):
        ...
        try:
            (script_tag, js_path, source, docstring, height_hint) = self.process_source(
                source, path, js_filename
            )
        ...
        intro = (
            self.parse(docstring, "<bokeh-content>")
            if docstring and process_docstring
            else []
        )
...
def _evaluate_source(source: str, filename: str, env):
    ...
    if len(d.roots) != 1:
        raise RuntimeError(
            f"bokeh-plot:: directive expects a single Document root, got {len(d.roots)}"
        )
...
def _remove_module_docstring(source, docstring):
    if docstring is None:
        return source
    return re.sub(
        rf"(\'\'\'|\"\"\")\s*{re.escape(docstring)}\s*(\'\'\'|\"\"\")", "", source
    )
