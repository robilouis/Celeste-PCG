...
class BokehModelDirective(BokehDirective):
    ...
    def run(self):
        ...
        if not issubclass(model, Model):
            raise SphinxError(
                f"Unable to generate model reference docs: {model_name}, is not a subclass of Model"
            )
        ...
        model_json = json.dumps(
            to_json_rep(model_obj), sort_keys=True, indent=2, separators=(", ", ": ")
        )

        # we only want to document things as coming from top-level `bokeh.models`
        adjusted_module_name = (
            "bokeh.models" if module_name.startswith("bokeh.models") else module_name
        )
...
def to_json_rep(obj: Model) -> dict[str, AnyRep]:
    ...
    properties = obj.properties_with_values(
        include_defaults=True, include_undefined=True
    )
