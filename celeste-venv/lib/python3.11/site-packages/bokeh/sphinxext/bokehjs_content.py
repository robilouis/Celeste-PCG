...
class bokehjs_content(nodes.General, nodes.Element):
    ...
    def visit_html(visitor, node):
        ...
        visitor.body.append(
            BJS_PROLOGUE.render(id=node["target_id"], title=node["title"])
        )
...
class BokehJSContent(CodeBlock):
    ...
    def get_js_source(self):
        ...
        if js_file:
            log.debug(
                f"[bokehjs-content] handling external example in {self.env.docname!r}: {js_file}"
            )
            path = js_file
            ...
            log.debug(
                f"[bokehjs-content] handling inline example in {self.env.docname!r}"
            )
            js_source = "\n".join(self.content)
