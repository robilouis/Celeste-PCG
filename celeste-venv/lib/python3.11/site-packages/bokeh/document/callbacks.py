...
class DocumentCallbackManager:
    ...
    def add_session_callback(
        ...
        if doc is None:
            raise RuntimeError(
                "Attempting to add session callback to already-destroyed Document"
            )
    ...
    def notify_event(
        self, model: Model, event: ModelEvent, callback_invoker: Invoker
    ) -> None:
        """ """
        doc = self._document()
        if doc is None:
    ...
    def _on_event(
        self, event: str | type[Event], *callbacks: EventCallback | JSEventCallback
    ) -> None:
    ...
    def event_callbacks_for_event_name(
        self, event_name: str
    ) -> tuple[EventCallback, ...]:
...
def invoke_with_curdoc(doc: Document, f: Callable[[], None]) -> None:
    ...
    curdoc: Document | UnlockedDocumentProxy = (
        UnlockedDocumentProxy(doc) if getattr(f, "nolock", False) else doc
    )
...
def _combine_document_events(
    new_event: DocumentChangedEvent, old_events: list[DocumentChangedEvent]
) -> None:
