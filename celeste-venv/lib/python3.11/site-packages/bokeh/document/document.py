...
class Document:
    ...
    def __init__(
        self, *, theme: Theme = default_theme, title: str = DEFAULT_TITLE
    ) -> None:
    ...
    def session_destroyed_callbacks(
        self, callbacks: set[SessionDestroyedCallback]
    ) -> None:
    ...
    def theme(self, theme: Theme | str | None) -> None:
        ...
        if isinstance(theme, str):
            try:
                ...
                raise ValueError(
                    f"{theme} is not a built-in theme; available themes are {nice_join(built_in_themes)}"
                )
    ...
    def add_periodic_callback(
        self, callback: Callback, period_milliseconds: int
    ) -> PeriodicCallback:
        ...
        cb = PeriodicCallback(
            callback=None, period=period_milliseconds, callback_id=make_id()
        )
    ...
    def add_timeout_callback(
        self, callback: Callback, timeout_milliseconds: int
    ) -> TimeoutCallback:
        ...
        cb = TimeoutCallback(
            callback=None, timeout=timeout_milliseconds, callback_id=make_id()
        )
        return self.callbacks.add_session_callback(cb, callback, one_shot=True)

    def apply_json_patch(
        self,
        patch_json: PatchJson | Serialized[PatchJson],
        *,
...
"""
    ...
    def js_on_event(
        self, event: str | type[Event], *callbacks: JSEventCallback
    ) -> None:
    ...
    def select_one(self, selector: SelectorType) -> Model | None:
        ...
        if len(result) > 1:
            raise ValueError(
                f"Found more than one model matching {selector}: {result!r}"
            )
        if len(result) == 0:
            return None
        return result[0]
    ...
    def set_select(
        self, selector: SelectorType | type[Model], updates: dict[str, Any]
    ) -> None:
