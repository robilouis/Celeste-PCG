...
class DocumentModelManager:
    ...
    def get_one_by_name(self, name: str) -> Model | None:
        ...
        """
        return self._models_by_name.get_one(
            name, f"Found more than one model named '{name}'"
        )

    @property
    def synced_references(self) -> set[Model]:
        return set(
            model for model in self._models.values() if model not in self._new_models
        )
    ...
    def flush_synced(self, is_still_new: Callable[[Model], bool] | None = None) -> None:
        ...
        if is_still_new is None:
            ...
            self._new_models = set(
                new_model for new_model in self._new_models if is_still_new(new_model)
            )
    ...
    def update_name(
        self, model: Model, old_name: str | None, new_name: str | None
    ) -> None:
