...
class BokehTornado(TornadoApplication):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        log.debug(
            f"These host origins can connect to the websocket: {list(self._websocket_origins)!r}"
        )
        ...
        for url, app in applications.items():
            assert isinstance(
                app, Application
            )  # TODO: unnecessary; improve type flow to remove this
            self._applications[url] = ApplicationContext(
                app, url=url, logout_url=self.auth_provider.logout_url
            )
        ...
        for key, ctx in self._applications.items():
            ...
            all_patterns.append(
                create_static_handler(self._prefix, key, ctx.application)
            )
    ...
    def _log_mem(self) -> None:
        ...
        objs = [
            ...
            if isinstance(x, ModuleType) and "bokeh_app_" in str(x)
        ]
