    ...
    async def get_session(self) -> ServerSession:
        ...
        if "Bokeh-Session-Id" in self.request.headers:
            if session_id is not None:
                log.debug(
                    "Server received session ID in request argument and header, expected only one"
                )
                raise HTTPError(
                    status_code=403,
                    reason="session ID was provided as an argument and header",
                )
        ...
        if token is not None:
            if session_id is not None:
                log.debug(
                    "Server received both token and session ID, expected only one"
                )
                raise HTTPError(
                    status_code=403, reason="Both token and session ID were provided"
                )
        ...
        if token is None:
            ...
            headers = {
                k: v for k, v in self.request.headers.items() if k in allowed_headers
            }
            ...
            payload.update(
                self.application_context.application.process_request(self.request)
            )
        ...
        if not check_token_signature(
            token, secret_key=app.secret_key, signed=app.sign_sessions
        ):
