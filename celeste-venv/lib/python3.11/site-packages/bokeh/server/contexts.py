...
class BokehSessionContext(SessionContext):
    ...
    async def with_locked_document(
        self, func: Callable[[Document], Awaitable[None]]
    ) -> None:
...
class ApplicationContext:
    ...
    async def _discard_session(
        self, session: ServerSession, should_discard: Callable[[ServerSession], bool]
    ) -> None:
        ...
        def do_discard() -> None:
            ...
            if should_discard(session) and session.expiration_blocked_count == 1:
                ...
                log.warning(
                    f"Session {session.id!r} was scheduled to discard but came back to life"
                )
...
class _RequestProxy:
    ...
    def __init__(
        ...
        if cookies is not None:
            ...
            self._cookies = {
                k: v if isinstance(v, str) else v.value
