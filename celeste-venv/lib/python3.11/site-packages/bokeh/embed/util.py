...
def OutputDocumentFor(
    objs: Sequence[Model],
    apply_theme: Theme | type[FromCurdoc] | None = None,
...
class RenderItem:
    def __init__(
        ...
        if (docid is None and token is None) or (
            docid is not None and token is not None
        ):
...
def standalone_docs_json_and_render_items(
    models: Model | Document | Sequence[Model | Document],
    *,
...
def _set_temp_theme(
    doc: Document, apply_theme: Theme | type[FromCurdoc] | None
) -> None:
