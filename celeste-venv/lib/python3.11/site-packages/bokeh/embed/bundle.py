...
class Bundle:
    ...
    def _render_js(self) -> str:
        return JS_RESOURCES.render(
            js_files=self.js_files, js_raw=self.js_raw, hashes=self.hashes
        )
...
def bundle_for_objs_and_resources(
    objs: Sequence[Model | Document] | None, resources: Resources | None
) -> Bundle:
    ...
    if resources is not None:
        ...
        if mode == "inline":
            js_raw.extend(
                [Resources._inline(bundle.artifact_path) for bundle in extensions]
            )
        elif mode == "server":
    ...
    return Bundle(
        js_files, js_raw, css_files, css_raw, resources.hashes if resources else {}
    )
...
def _query_extensions(
    all_objs: set[Model], query: Callable[[type[Model]], bool]
) -> bool:
    names: set[str] = set()
...
def _bundle_extensions(
    objs: set[Model] | None, resources: Resources
) -> list[ExtensionEmbed]:
    names: set[str] = set()
...
def _use_tables(all_objs: set[Model]) -> bool:
    ...
    return _any(all_objs, lambda obj: isinstance(obj, TableWidget)) or _ext_use_tables(
        all_objs
    )
...
def _use_widgets(all_objs: set[Model]) -> bool:
    ...
    return _any(all_objs, lambda obj: isinstance(obj, Widget)) or _ext_use_widgets(
        all_objs
    )
...
def _use_mathjax(all_objs: set[Model]) -> bool:
    ...
    return _any(
        all_objs, lambda obj: isinstance(obj, MathText) or _model_requires_mathjax(obj)
    ) or _ext_use_mathjax(all_objs)
...
def _use_gl(all_objs: set[Model]) -> bool:
    ...
    return _any(
        all_objs, lambda obj: isinstance(obj, Plot) and obj.output_backend == "webgl"
    )
