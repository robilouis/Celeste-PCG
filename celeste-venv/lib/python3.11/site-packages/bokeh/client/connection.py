...
class ClientConnection:
    ...
    async def _connect_async(self) -> None:
        ...
        if self._socket is None:
            await self._transition_to_disconnected(
                DISCONNECTED(ErrorReason.NETWORK_ERROR, None, "Socket invalid.")
            )
        else:
    ...
    async def _handle_messages(self) -> None:
        ...
        if message is None:
            await self._transition_to_disconnected(
                DISCONNECTED(ErrorReason.HTTP_ERROR, 500, "Internal server error.")
            )
        else:
    ...
    def _send_message_wait_for_reply(
        ...
        async def handle_message(
            message: Message[Any], send_result: list[None]
        ) -> None:
