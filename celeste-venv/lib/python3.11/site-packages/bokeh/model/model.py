...
class Model(HasProps, HasDocumentRef, PropertyCallbackManager, EventCallbackManager):
    ...
    def __new__(cls, *args: Any, id: ID | None = None, **kwargs: Any) -> Model:
        ...
        if id is not None:
            if args or kwargs:
                raise ValueError(
                    "'id' cannot be used together with property initializers"
                )
            obj._id = id
        else:
            obj._id = make_id()
    ...
    def js_link(
        ...
        other: Model,
        other_attr: str,
        attr_selector: int | str | None = None,
    ) -> None:
        ...
        cb = CustomJS(
            ...
            code=f"other.{other_descriptor.name} = this.{descriptor.name}{selector}",
        )
    ...
    def select_one(self, selector: SelectorType) -> Model | None:
        ...
        if len(result) > 1:
            raise ValueError(
                f"Found more than one object matching {selector}: {result!r}"
            )
        if len(result) == 0:
            return None
        return result[0]
    ...
    def set_select(
        self, selector: type[Model] | SelectorType, updates: dict[str, Any]
    ) -> None:
    ...
    def _attach_document(self, doc: Document) -> None:
        ...
        if self.document is not None:
            raise RuntimeError(
                f"Models must be owned by only a single document, {self!r} is already in a doc"
            )
