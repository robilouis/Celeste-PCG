...
class _ColorGroupMeta(type):
    ...
    def __getitem__(self, v: str | int) -> NamedColor:
        ...
        if isinstance(v, str):
            ...
            raise KeyError(
                f"Color group {self.__class__.__name__!r} has no color {v!r}"
            )
        if isinstance(v, int):
            if v >= 0 and v < len(self):
                return getattr(named, self._colors[v].lower())
            raise IndexError(
                f"Index out of range for color group {self.__class__.__name__!r}"
            )
        raise ValueError(
            f"Unknown index {v!r} for color group {self.__class__.__name__!r}"
        )
...
class NamedColor(RGB):
    ...
    def from_string(cls, color: str) -> RGB:
        ...
        try:
            ...
            if rgb is None:
                raise ValueError(
                    f"Color '{color}' must be either a named color or an RGB(A) hex string"
                )
