...
def server_url(
    host: str | None = None, port: int | None = None, ssl: bool = False
) -> str:
    ...
    * ``'absolute-dev'`` same as ``absolute`` but supports non-minified assets
    ...
    def __init__(
        ...
        *,
        ...
        self.components = (
            components if components is not None else list(self._default_components)
        )
    ...
    def _file_paths(self, kind: Kind) -> list[str]:
        ...
        files = [
            f"{component}{minified}.{kind}" for component in self.components_for(kind)
        ]
    ...
    def render_js(self) -> str:
        return JS_RESOURCES.render(
            js_raw=self.js_raw, js_files=self.js_files, hashes=self.hashes
        )
...
class SessionCoordinates:
    ...
    def __init__(
        self, *, url: str = DEFAULT_SERVER_HTTP_URL, session_id: ID | None = None
    ) -> None:
        self._url = url
...
def _get_cdn_urls(version: str | None = None, minified: bool = True) -> Urls:
    ...
    if is_full_release(version):  # TODO: TypeGuard?
        ...
        result.hashes = lambda components, kind: {
            mk_url(component, kind): sri_hashes[mk_filename(component, kind)]
