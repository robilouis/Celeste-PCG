...
def visit_Call(
    ...
    if (
        ...
        if (
            ...
            if node.args[1].value in MODE_REPLACE or (
            ...
            if (
                mode is not None
                and isinstance(mode.value, ast.Constant)
                and isinstance(mode.value.value, str)
                and (
                    mode.value.value in MODE_REMOVE or mode.value.value in MODE_REPLACE
                )
            ):
