...
def _eligible_lambda_replacement(lambda_expr: ast.Lambda) -> str | None:
    if isinstance(lambda_expr.body, ast.Constant):
        ...
        isinstance(lambda_expr.body, ast.Call)
        and isinstance(lambda_expr.body.func, ast.Name)
        and not lambda_expr.body.args
        and not lambda_expr.body.keywords
        and lambda_expr.body.func.id in {"dict", "list", "set", "tuple"}
    ):
