...
class Test(unittest.TestCase):
    ...
    def _do_analyze(self, files):
        ...
        p = subprocess.Popen(
            [sys.executable, self._coverage_file, "--pydev-analyze"],
    ...
    def test_pydev_analyse_non_standard_encoding(self):
        ref_valid_files = [
            os.path.join(
                ...
                "_pydev_coverage_cyrillic_encoding_py%i.py" % sys.version_info[0],
            )
        ]
        ref_invalid_files = []
    ...
    def test_pydev_analyse_invalid_files(self):
        with tempfile.NamedTemporaryFile(suffix=".pyx") as pyx_file:
            ref_valid_files = []
            ref_invalid_files = [
                os.path.join(self._resources_path, "_pydev_coverage_syntax_error.py"),
                pyx_file.name,
            ]
