...
if __name__ == "__main__":
    if "-print-debug" in sys.argv:
        ...
        log_contents = open(
            pydev_log._compute_filename_with_pid(target_log_file)
        ).read()
        assert (
            "Something in initial" in log_contents
        ), 'Did not find "Something in initial" in %s' % (log_contents,)

        log_contents = open(
            pydev_log._compute_filename_with_pid(target_log_file, pid=subprocess_pid)
        ).read()
        assert (
            "Something in print-debug" in log_contents
        ), 'Did not find "Something in print-debug" in %s' % (log_contents,)

        output = "".join(output.splitlines(keepends=True)[1:])  # Remove the first line
        loaded_debug_info = json.loads(output)
        assert loaded_debug_info == new_debug_info, "Expected %s. Found: %s" % (
            new_debug_info,
            loaded_debug_info,
        )
