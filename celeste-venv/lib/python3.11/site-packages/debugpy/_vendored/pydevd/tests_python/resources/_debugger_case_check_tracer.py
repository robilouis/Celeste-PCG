...
def check_frame_info(expected):
    ...
    for found_info, expected_info in zip(found, expected):
        if (
            found_info.filename != expected_info.filename
            or found_info.name != expected_info.name
        ):
            fail()
...
def thread_func():
    check_frame_info(
        [
            FrameInfo(
                filename="_debugger_case_check_tracer.py",
                name="thread_func",
                f_trace="trace_exception",
            ),
            FrameInfo(filename="threading.py", name="run", f_trace="None"),
            FrameInfo(
                filename="threading.py",
                name="_bootstrap_inner",
                f_trace="trace_unhandled_exceptions",
            ),
...
check_frame_info(
    [
        ...
        FrameInfo(
            filename="pydevd.py", name="_exec", f_trace="trace_unhandled_exceptions"
        ),
