...
def test_in_project_roots(tmpdir):
    ...
    if IS_WINDOWS:
        assert files_filtering._get_library_roots() == [
            os.path.normcase(x) + "\\"
            for x in files_filtering._get_default_library_roots()
        ]
    elif IS_MAC:
        assert files_filtering._get_library_roots() == [
    ...
    for check_path, find in check:
        assert (
            files_filtering.in_project_roots(check_path) == find
    ...
    try:
        ...
        assert len(set(default_library_roots)) == len(
...
def test_rules_to_exclude_filter(tmpdir):
    from _pydevd_bundle.pydevd_process_net_command_json import (
        _convert_rules_to_exclude_filters,
    )
    from _pydevd_bundle.pydevd_filtering import ExcludeFilter
