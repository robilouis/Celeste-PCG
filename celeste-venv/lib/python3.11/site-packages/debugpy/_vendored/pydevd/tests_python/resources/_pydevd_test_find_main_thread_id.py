...
def check_main_thread_id_simple():
    ...
    assert main_thread_id == thread.get_ident(), "Found: %s, Expected: %s" % (
        main_thread_id,
        thread.get_ident(),
    )
    assert not log_msg
...
def check_fix_main_thread_id_multiple_threads():
    ...
    def method():
        ...
        assert threading.current_thread() is attach_script.get_main_thread_instance(
            threading
        )
        ...
        assert threading.current_thread().ident == thread.get_ident()
        assert threading.current_thread() is not attach_script.get_main_thread_instance(
            threading
        )
